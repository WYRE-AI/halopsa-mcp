...
describe("Cloudflare Worker entrypoint", () => {
  ...
  it("lists all tools without credentials", async () => {
    ...
    expect(names).toContain("halopsa_navigate");
    expect(names).toContain("halopsa_status");
    expect(names).toContain("halopsa_tickets_list");
  ...
  it("returns a graceful error for a credential-requiring tool when unconfigured", async () => {
    const res = await mcp({
      ...
      id: 3,
      method: "tools/call",
      params: { name: "halopsa_tickets_list", arguments: {} },
    });
    expect(res.status).toBe(200);
    const body = (await res.json()) as {
